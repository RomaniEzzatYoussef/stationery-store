...
DROP SCHEMA IF EXISTS `stationery-store-db`;
CREATE SCHEMA `stationery-store-db`;
...
(
    ...
    `user_id` int(11) NOT NULL,
    PRIMARY KEY (`id`),
    CONSTRAINT `FK_USER_CART` FOREIGN KEY (`user_id`) REFERENCES `user` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE = InnoDB
  AUTO_INCREMENT = 1
  DEFAULT CHARSET = latin1;
